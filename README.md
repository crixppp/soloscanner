...
To view the site locally, open `site/index.html` in a browser or serve the directory with any static web server. When hosted on GitHub Pages, make sure the Pages source is set to the `/site` folder (Settings → Pages → Build and deployment). A lightweight `index.html` lives at the repository root to redirect visitors to `/site/` in case Pages is accidentally pointed at the repository root.
